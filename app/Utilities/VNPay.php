...
class VNPay
{
     ...
     *
     */
    static $vnp_TmnCode = "J58JFBC6"; //Mã website tại VNPAY
    static $vnp_HashSecret ="3SSGI6ZBGIJJEGODNKJQDO05JGZYAD8A"; //Chuỗi bí mật
    ...
    public static function vnpay_create_payment(array $data)
    {
        ...
        $vnp_Amount_VND = (int)$data['vnp_Amount'];
      
        $vnp_Amount = $vnp_Amount_VND * 100;
    ...
    //      $vnp_BankCode = 'NCB';
        ...
        $inputData = array(
            ...
            "vnp_ReturnUrl" => rtrim(config('app.url'), '/') . self::$vnp_Returnurl,
        ...
        ksort($inputData);
        $hashdata = http_build_query($inputData); // Tạo chuỗi: key1=value1&key2=value2...

        // VNPAY yêu cầu Hash trên chuỗi HashData + HashSecret
        $vnpSecureHash = hash_hmac('sha512', $hashdata, self::$vnp_HashSecret);

        // URL phải chứa các tham số đã URL encode
        $query = http_build_query($inputData); 
        $vnp_Url = self::$vnp_Url . "?" . $query . '&vnp_SecureHashType=SHA512&vnp_SecureHash=' . $vnpSecureHash;
